...
| --- | --- | ---: | ---: | ---: |
| source-001 | **예** | — | — | — |
| source-002 | 아니오 | 0.532 | 1.416 | 0.567 |
| source-003 | 아니오 | 0.476 | 1.349 | 0.595 |
| source-004 | **예** | — | — | — |

### 판정

사람이 만든 한국어 자막은 **머뭇거림의 약 절반을 삭제**했다. 필러 밀도는 1000자당
5.7~8.8인 반면 자동 caption은 16.5~24.6이다.

**따라서 사람 자막은 `raw_transcript`가 될 수 없다.** `normalized_transcript`와 정렬
참고자료로 쓴다.

한편 사람 자막의 문자 수는 자동 caption보다 35~42% **많다.** 최초 측정에서는 반대로
나왔고 그것은 버그였다. 사람 자막이 내용을 잘라낸다는 근거는 없다. 필러를 지우면서도
전체 분량은 더 많으므로, 자동 caption이 놓친 발화를 담고 있을 가능성이 있다.

### 따라오는 결론

사람 자막이 있는 source와 없는 source의 전사 비용은 같다. 어느 쪽이든
`raw_transcript`를 위해서는 STT가 필요하기 때문이다.
...
- 문장부호 밀도는 **같은 source 안의 두 트랙을 비교할 때만** 편집의 신호다. 최신
  YouTube ASR은 문장부호를 붙이므로 source끼리 비교하면 생성 시점 차이를 편집으로
  오독한다 ([ADR-0003 §E3](adr/0003-reference-transcription-strategy.md))
