...
class JsonIdentityRepository:
    ...
    def load(self) -> CompanionIdentity:
        try:
            ...
            identity = CompanionIdentity(
                raw["name"],
                raw["role"],
                tuple(raw["core_personality"]),
                tuple(raw["values"]),
                tuple(raw["relationship_policy"]),
                tuple(raw["immutable_boundaries"]),
                raw["version"],
            )
        except (OSError, KeyError, TypeError, json.JSONDecodeError) as error:
            raise IdentityRepositoryError(
                f"could not load identity from {self._path}: {error}"
            ) from error
        _validate(identity)
        return identity
    ...
    def save(self, identity: CompanionIdentity) -> None:
        ...
        try:
            ...
            raise IdentityRepositoryError(
                f"could not save identity to {self._path}: {error}"
            ) from error


def _validate(identity: CompanionIdentity) -> None:
    required_strings = (identity.name, identity.role, identity.version)
    required_groups = (
        identity.core_personality,
        identity.values,
        identity.relationship_policy,
        identity.immutable_boundaries,
    )
    if not all(value.strip() for value in required_strings) or not all(
        group and all(value.strip() for value in group) for group in required_groups
    ):
